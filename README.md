...
- **🧭 자연어 검색 웹앱 ([`app/`](app/))** — 하고 싶은 걸 한국어로 입력하면 의미로 맞는 API를 찾아주고 호출 코드(fetch/curl/python)까지 생성합니다. 브라우저 로컬 임베딩 의미 검색 + 무료 LLM 솔루션 설계 + 카테고리 탐색/정렬 + 라이브 상태 배지 + 라이트/다크 테마 + PWA/즐겨찾기. **`master`에 머지되면 GitHub Pages로 자동 배포**됩니다 → `https://kmini0157.github.io/open-apis-korea/` (기타 호스팅은 [`DEPLOY.md`](DEPLOY.md)).
